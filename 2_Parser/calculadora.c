
#include <stdlib.h>
#include <stdio.h>
#include <stdarg.h>
#include <string.h>
#include <math.h>

#include "calculadora.h"

/* symbol table */
/* hash a symbol */
static unsigned symhash(char * sym) { 
	unsigned int hash = 0;
	unsigned c;
	// while( c = *sym ++ ) hash = hash * 9 ^ c;
	while( !(c = (*sym ++)) ) hash = hash * 9 ^ c;
	return hash;
}

struct symbol * lookup(char * sym) {
	struct symbol * sp = & symtab[ symhash(sym) % NHASH ];
	int scount = NHASH; /* how many have we looked at */
	while( --scount >= 0 ) {
		if ( sp->name && !strcmp(sp->name, sym) ) {
			return sp;
		}
		if ( ! sp->name ) { /* new entry */
			sp->name = strdup(sym);
			sp->value = 0;
			sp->func = NULL;
			sp->syms = NULL;
			return sp;
		}
		if ( ++sp >= symtab+NHASH ) sp = symtab; /* try the next entry */
	}

	yyerror("symbol table overflow\n");
	abort(); /* tried then all, table is full */
}


struct ast * newast( int nodetype, struct ast * left, struct ast * right ) {

	struct ast * n = (struct ast *) malloc(sizeof (struct ast));

	if ( !n ) {
		yyerror("out of memory!!!");
		exit( -2 );
	}

	n->nodetype = nodetype;
	n->left = left;
	n->right = right;

	return n;
}

struct ast * newnum( double value ) {

	struct numval * n = (struct numval *) malloc(sizeof (struct numval));

	if ( !n ) {
		yyerror("out of memory!!!");
		exit( -1 );
	}

	n->nodetype = 'K';
	n->number = value;

	return (struct ast *) n;
}

struct ast * newcmp(int cmptype, struct ast *left, struct ast *right) {
	struct ast * a = (struct ast *) malloc( sizeof(struct ast) );
	if ( ! a ) {
		yyerror("out of space");
		exit( -2 );
	}
	a->nodetype = '0' + cmptype;
	a->left = left;
	a->right = right;
	return a;
}

struct ast * newfunc(int functype, struct ast *l) {
	struct fncall * a = (struct fncall *) malloc( sizeof(struct fncall) );
	if ( ! a ) {
		yyerror("out of space");
		exit( -2 );
	}
	a->nodetype = 'F';
	a->l = l;
	a->functype = functype;
	return (struct ast *) a;
}

struct ast * newcall(struct symbol *s, struct ast *l) {
	struct ufncall * a = (struct ufncall *) malloc( sizeof(struct ufncall) );
	if ( ! a ) {
		yyerror("out of space");
		exit( -2 );
	}
	a->nodetype = 'C';
	a->l = l;
	a->s = s;
	return (struct ast *) a;
}

struct ast * newref(struct symbol *s) {
	struct symref * a = (struct symref *) malloc( sizeof(struct symref) );
	if ( ! a ) {
		yyerror("out of space");
		exit( -2 );
	}
	a->nodetype = 'N';
	a->s = s;
	return (struct ast *) a;
}

struct ast * newasgn(struct symbol *s, struct ast *v) {
	struct symasgn * a = (struct symasgn *) malloc( sizeof(struct symasgn) );
	if ( ! a ) {
		yyerror("out of space");
		exit( -2 );
	}
	a->nodetype = '=';
	a->s = s;
	a->v = v;
	return (struct ast *) a;
}

struct ast * newflow(int nodetype, struct ast *cond, struct ast *tl, struct ast *tr) {
	struct flow * a = (struct flow *) malloc( sizeof(struct flow) );
	if ( ! a ) {
		yyerror("out of space");
		exit( -2 );
	}
	a->nodetype = nodetype;
	a->cond = cond;
	a->tl = tl;
	a->el = tr;
	return (struct ast *) a;
}

struct symlist * newsymlist(struct symbol * sym, struct symlist * next) {
	struct symlist * a = (struct symlist *) malloc( sizeof(struct symlist) );
	if ( ! a ) {
		yyerror("out of space");
		exit( -2 );
	}
	a->sym = sym;
	a->next = next;
	return a;
}

void symlistfree(struct symlist * sl) {
	struct symlist * nsl;
	while( sl ) {
		nsl = sl->next;
		free( sl );
		sl = nsl;
	}
}

char * getsymbols(struct symlist * sl ) {
	char * s = "[";
	struct symlist * nsl = NULL;
	while( sl != NULL ) {
		nsl = sl->next;
		//printf("%s\n", (sl->sym)->name);
		concatenar(s, (sl->sym)->name);
		if ( nsl != NULL ) concatenar(s, ",");
		sl = nsl;
	}
	concatenar(s, "]");
	return s;
}

void dodef(struct symbol *name, struct symlist *syms, struct ast *func) {
	if(name->syms) {
		symlistfree(name->syms);
	}
	if(name->func) {
		treefree(name->func);
	}
	name->syms = syms;
	name->func = func;
}

static double callbuiltin(struct fncall *f);
static double calluser(struct ufncall *f);

double eval( struct ast * n ) {

	double v = 0.0;
	if( !n ) {
	    yyerror("internal error, null eval");
	    return 0.0;
	}

	switch(n->nodetype) {

		/* constant */
		case 'K': v = ((struct numval *)n)->number; break;
		
		/* name reference */ 
		case 'N': v = ((struct symref *)n)->s->value;  break;
		
		/* assignment */
		case '=': v = ((struct symasgn *)n)->s->value = eval(((struct symasgn *)n)->v);  break;

		case '+': v = eval(n->left) + eval(n->right); break;
		case '-': v = eval(n->left) - eval(n->right); break;
		case '*': v = eval(n->left) * eval(n->right); break;
		case '/': v = eval(n->left) / eval(n->right); break;
		case '|': v = fabs(eval(n->left)); break;
		case 'M': v = - eval(n->left); break;

			/* comparisons */
		case '1': v = (eval(n->left) >  eval(n->right)) ? 1 : 0; break;
		case '2': v = (eval(n->left) <  eval(n->right)) ? 1 : 0; break;
		case '3': v = (eval(n->left) != eval(n->right)) ? 1 : 0; break;
		case '4': v = (eval(n->left) == eval(n->right)) ? 1 : 0; break;
		case '5': v = (eval(n->left) >= eval(n->right)) ? 1 : 0; break;
		case '6': v = (eval(n->left) <= eval(n->right)) ? 1 : 0; break;

		/* control flow */

		/* null if/else/do expressions allowed in the grammar, so check for them */
		case 'I': 
			if( eval( ((struct flow *)n)->cond) != 0) {
				if( ((struct flow *)n)->tl) {
					v = eval( ((struct flow *)n)->tl);
				} else v = 0.0;		/* a default value */
			} else {
				if( ((struct flow *)n)->el) {
					v = eval(((struct flow *)n)->el);
				} else v = 0.0;		/* a default value */
			}
		    break;

		case 'W':
		    v = 0.0;		/* a default value */
		    if( ((struct flow *)n)->tl) {
		    	while( eval(((struct flow *)n)->cond) != 0)
		    		v = eval(((struct flow *)n)->tl);
		    }
		    break;			/* last value is value */
			              
		  case 'L': eval(n->left); v = eval(n->right); break;

		  case 'F': v = callbuiltin((struct fncall *)n); break;

		  case 'C': v = calluser((struct ufncall *)n); break;

		default:
			printf("Erro interno: no desconhecido %c\n", n->nodetype);
	}

	return v;
}

static double callbuiltin(struct fncall *f) {
	enum bifs functype = f->functype;
	double v = eval(f->l);
	switch(functype) {
		case B_sqrt:
		   return sqrt(v);
		case B_exp:
		   return exp(v);
		case B_log:
		   return log(v);
		case B_print:
		   printf("= %4.4g\n", v);
		   return v;
		default:
		   yyerror("Unknown built-in function %d", functype);
		   return 0.0;
	 }
}

static double calluser(struct ufncall *f) {
  struct symbol *fn = f->s;	/* function name */
  struct symlist *sl;		/* dummy arguments */
  struct ast *args = f->l;	/* actual arguments */
  double *oldval, *newval;	/* saved arg values */
  double v;
  int nargs;
  int i;

  if(!fn->func) {
    yyerror("call to undefined function", fn->name);
    return 0;
  }

  /* count the arguments */
  sl = fn->syms;
  for(nargs = 0; sl; sl = sl->next) nargs++;

  /* prepare to save them */
  oldval = (double *)malloc(nargs * sizeof(double));
  newval = (double *)malloc(nargs * sizeof(double));
  if(!oldval || !newval) {
    yyerror("Out of space in %s", fn->name); 
    return 0.0;
  }
  
  /* evaluate the arguments */
  for(i = 0; i < nargs; i++) {
    if(!args) {
      yyerror("too few args in call to %s", fn->name);
      free(oldval); free(newval);
      return 0;
    }

    if(args->nodetype == 'L') {	/* if this is a list node */
      newval[i] = eval(args->left);
      args = args->right;
    } else {			/* if it's the end of the list */
      newval[i] = eval(args);
      args = NULL;
    }
  }
		     
  /* save old values of dummies, assign new ones */
  sl = fn->syms;
  for(i = 0; i < nargs; i++) {
    struct symbol *s = sl->sym;

    oldval[i] = s->value;
    s->value = newval[i];
    sl = sl->next;
  }

  free(newval);

  /* evaluate the function */
  v = eval(fn->func);

  /* put the dummies back */
  sl = fn->syms;
  for(i = 0; i < nargs; i++) {
    struct symbol *s = sl->sym;

    s->value = oldval[i];
    sl = sl->next;
  }

  free(oldval);
  return v;
}

void treefree( struct ast * n ) {
	switch( n->nodetype ) {
			/* two subtrees */
		case '+':
		case '-':
		case '*':
		case '/':
		case '1':
		case '2':
		case '3':
		case '4':
		case '5':
		case '6':
		case 'L':
			// treefree(n->left);
			treefree(n->right);
			//break;
			/* one subtree */
		case '|':
		case 'M':
		case 'C':
		case 'F':
			treefree(n->left);
			break;
			/* no subtree */
		case 'K':
		case 'N':
			//free(n);
			break;

		case '=':
			free( ((struct symasgn *)n)->v);
			break;
			/* up to three subtrees */
		case 'I':
		case 'W':
			free( ((struct flow *)n)->cond);
			if ( ((struct flow *)n)->tl) treefree( ((struct flow *)n)->tl);
			if ( ((struct flow *)n)->el) treefree( ((struct flow *)n)->el);
			break;
		default:
			printf("Internal error: free bad node %c\n", n->nodetype);
	}
	free( n ); // always free the node itself
}

void yyerror(char * s, ...) {
	va_list ap;
	va_start(ap, s);

	fprintf(stderr, "%d : error: ", yylineno );
	vfprintf(stderr, s, ap);
	fprintf(stderr, "\n" );
}

/* debugging: dump out an AST */
int debug = 0;
void dumpast(struct ast * a, int level) {

  printf("%*s", 2 * level, "");	/* indent to this level */
  level++;

  if( !a ) {
    printf("NULL\n");
    return;
  }

  switch(a->nodetype) {
    /* constant */
  case 'K': printf("number %4.4g\n", ((struct numval *)a)->number); break;

    /* name reference */
  case 'N': printf("ref %s\n", ((struct symref *)a)->s->name); break;

    /* assignment */
  case '=': 
  	printf("= %s\n", ((struct symref *)a)->s->name);
    dumpast( ((struct symasgn *)a)->v, level); 
    return;

    /* expressions */
  case '+': 
  case '-': 
  case '*': 
  case '/': 
  case 'L':
  case '1': 
  case '2': 
  case '3':
  case '4': 
  case '5': 
  case '6': 
    printf("binop %c\n", a->nodetype);
    dumpast(a->left, level);
    dumpast(a->right, level);
    return;

  case '|': 
  case 'M': 
    printf("unop %c\n", a->nodetype);
    dumpast(a->left, level);
    return;

  case 'I': 
  case 'W':
    printf("flow %c\n", a->nodetype);
    dumpast( ((struct flow *)a)->cond, level);
    if( ((struct flow *)a)->tl) dumpast( ((struct flow *)a)->tl, level);
    if( ((struct flow *)a)->el) dumpast( ((struct flow *)a)->el, level);
    return;
	              
  case 'F':
    printf("builtin %d\n", ((struct fncall *)a)->functype);
    dumpast(a->left, level);
    return;

  case 'C': 
  	printf("call %s\n", ((struct ufncall *)a)->s->name);
    dumpast(a->left, level);
    return;

  default: printf("bad %c\n", a->nodetype);
    return;
  }
}


static char * concatenar(const char * s1, const char * s2) {
	char * c = (char *) malloc( (strlen(s1) + strlen(s2)) * sizeof(char) ); 
	if ( ! c ) {
		return "";
	}
	strcat(c, s1);
	strcat(c, s2);
	return c;
}
